...
# 2️⃣ LOAD EXCEL FILE
# ============================================================
excel_file = "tank_block_consolidated_report_2025-12-09_colored.xlsx"
...
# -----------------------------------------
# ⭐ DAILY & WEEKLY COMPLIANCE CALCULATION
# -----------------------------------------
# Always define ranges
ph_min, ph_max = 7.6, 8.0  # modify if needed
sal_min, sal_max = 25, 29  # modify if needed

# Default percentages so variables always exist
ph_percentage = 0.0
sal_percentage = 0.0
ph_total_count = 0
sal_total_count = 0

# Use the already-filtered view_df for both Daily and Weekly
if not view_df.empty:
    ...
    if "pH" in view_df.columns:
        ...
        ph_valid_count = view_df[
            (view_df["pH"] >= ph_min) & (view_df["pH"] <= ph_max)
        ].shape[0]
    ...
    if "Salinity" in view_df.columns:
        ...
        sal_valid_count = view_df[
            (view_df["Salinity"] >= sal_min) & (view_df["Salinity"] <= sal_max)
        ].shape[0]

        sal_percentage = (sal_valid_count / sal_total_count * 100) if sal_total_count > 0 else 0.0

# -----------------------------------------
# ⭐ Display Side-by-Side Compliance Cards
# -----------------------------------------
...
if not view_df.empty and metrics_present:
    ...
    id_vars = ['X_label'] + [c for c in ['Tank', 'Block', 'WorkerName'] if c in view_df.columns]
...
if not view_df.empty and metrics:
    ...
    if 'WorkerName' not in df_melt_metrics.columns:
        df_melt_metrics['WorkerName'] = 'Unknown'
    ...
    for metric in metrics:
        ...
        df_metric['RiskColor'] = df_metric['Value'].apply(
            lambda v: get_risk_color(metric, v)
        )
        ...
        for color in risk_points['RiskColor'].unique():
            ...
            fig_water.add_scatter(
                ...
                customdata=subset[['Tank', 'Block', 'WorkerName']].to_numpy(),
                hovertemplate=(
                    "Date: %{x}<br>"
                ...
                ).replace("%{marker.color}", "5" if color == "red" else "4")
            )
...
if not df_feed.empty:
    ...
    if 'Feed_Risk' in view_df.columns:
        ...
        if not risk_points.empty:
            # Ensure Tank/Block/Worker exist
...
if 'Block' not in df_mort.columns:
    df_mort['Block'] = '-'
if 'WorkerName' not in df_mort.columns:
    df_mort['WorkerName'] = 'Unknown'
...
if not view_df.empty:
    ...
    # ------------------------------
    def color_ph(val):
        if val < 7.6 or val > 8.0:  # high risk
            return "background-color: #FF0000"
        elif val < 7.7 or val > 7.9:  # medium risk
