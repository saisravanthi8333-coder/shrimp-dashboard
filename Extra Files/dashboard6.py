...
# Compute Risk Flags
# ------------------------------
view_df['pH_Risk'] = ((view_df['pH'] < 7.8) | (view_df['pH'] > 8.0)).astype(int)
view_df['Salinity_Risk'] = ((view_df['Salinity'] < 26) | (view_df['Salinity'] > 28)).astype(int)
view_df['Mortality_Risk'] = (view_df['Mortality_pct'] > 0).astype(int)
view_df['Feed_Risk'] = ((view_df['ScheduledFeed_day_g'] - view_df['ActualFeed_day_g']) > 0).astype(int)
view_df['Feed_Leftover_Risk'] = view_df['Feed_Risk']
view_df['Risk_Level'] = view_df[['pH_Risk','Salinity_Risk','Mortality_Risk','Feed_Risk','Feed_Leftover_Risk']].sum(axis=1)
...
if not view_df.empty:
    metrics = ["Salinity", "pH"]
    df_melt = view_df.melt(id_vars=["X_label", "Tank", "Block"], value_vars=metrics, var_name="Metric", value_name="Value")
    fig_water = px.line(
        df_melt,
        x="X_label",
        y="Value",
        ...
        hover_data={"Block": True, "Tank": True, "Metric": True, "Value": True, "X_label": True},
        color_discrete_map=tank_colors
    )
    # Add Risk 5 markers (pH or Salinity)
    risk5_water = view_df[((view_df['pH_Risk'] + view_df['Salinity_Risk']) == 2) & (view_df['Risk_Level']==5)]
    if not risk5_water.empty:
        for metric in ['pH','Salinity']:
            temp = risk5_water[['X_label','Tank','Block',metric]]
            fig_water.add_scatter(
                x=temp['X_label'], y=temp[metric],
                mode='markers',
                marker=dict(symbol='triangle-up', size=12, color="red"),
                name=f'{metric} Risk 5',
                hovertemplate="Date: %{customdata[0]}<br>Tank: %{customdata[1]}<br>Block: %{customdata[2]}<br>Value: %{y}<br>Risk Level: 5",
                customdata=temp[['X_label','Tank','Block']].to_numpy()
            )
...
if not view_df.empty:
    metrics = ["ScheduledFeed_day_g", "ActualFeed_day_g"]
    df_melt = view_df.melt(id_vars=["X_label","Tank","Block"], value_vars=metrics, var_name="Metric", value_name="Value")
    fig_feed = px.line(
        df_melt,
        x="X_label",
    ...
    # Feed Risk 5 markers
    feed_risk5 = view_df[(view_df['Feed_Risk']==1) & (view_df['Risk_Level']==5)]
    if not feed_risk5.empty:
        for metric in ['ActualFeed_day_g','ScheduledFeed_day_g']:
            temp = feed_risk5[['X_label','Tank','Block',metric]]
...
if not view_df.empty:
    metrics = ["DeadCount_day","Mortality_pct"]
    df_melt = view_df.melt(id_vars=["X_label","Tank","Block"], value_vars=metrics, var_name="Metric", value_name="Value")
    fig_mort = px.line(
        df_melt,
        x="X_label",
    ...
    # Mortality Risk 5 markers
    mort_risk5 = view_df[(view_df['Mortality_Risk']==1) & (view_df['Risk_Level']==5)]
    if not mort_risk5.empty:
        for metric in ['Mortality_pct','DeadCount_day']:
            temp = mort_risk5[['X_label','Tank','Block',metric]]
