...
#1️⃣ LOAD DATA
# =============================
excel_file = "tank_block_consolidated_report.xlsx"
...
if view_option == "Daily":
    ...
    if selected_tank != "All":
        filtered_df = filtered_df[filtered_df['Tank'] == selected_tank]
...
elif view_option == "Weekly" and selected_week != "All":
    # Ensure Week_Start & Week_End exist
    filtered_df['Week_Start'] = filtered_df['Date'] - pd.to_timedelta(filtered_df['Date'].dt.weekday, unit='d')
    filtered_df['Week_End'] = filtered_df['Week_Start'] + pd.Timedelta(days=6)
    week_start, week_end = selected_week.split(" to ")
    week_start = pd.to_datetime(week_start)
    week_end = pd.to_datetime(week_end)
    filtered_df = filtered_df[(filtered_df['Date'] >= week_start) & (filtered_df['Date'] <= week_end)]

elif view_option == "Monthly" and selected_month != "All":
    filtered_df = filtered_df[filtered_df['Month'].astype(str) == selected_month]

# Make view_df for charts and calculations
...
if view_option == "Weekly":
    ...
    weekly_df = view_df.groupby(['Block','Tank'], as_index=False).agg(
    ...
    weekly_df['ScheduledFeed_kg'] = (weekly_df['ScheduledFeed_g']/1000).round(2)
    weekly_df['ActualFeed_kg'] = (weekly_df['ActualFeed_g']/1000).round(2)
    weekly_df['Leftover_kg'] = (weekly_df['LeftoverFeed_g']/1000).round(2)

    # Metrics summary
    col1, col2, col3, col4, col5, col6 = st.columns(6)
    if not weekly_df.empty:
        col1.metric("Feed (kg)", f"{weekly_df['ActualFeed_kg'].sum():.2f}")
        col2.metric("Scheduled Feed (kg)", f"{weekly_df['ScheduledFeed_kg'].sum():.2f}")
        col3.metric("Leftover Feed (kg)", f"{weekly_df['Leftover_kg'].sum():.2f}")
        col4.metric("Dead Count", weekly_df['DeadCount'].sum())
        col5.metric("Dead Weight (g)", weekly_df['DeadWeight_g'].sum())
        col6.metric("Avg Mortality %", round(weekly_df['Mortality_pct'].mean()*100,2))
    else:
        for c in [col1, col2, col3, col4, col5, col6]:
            c.metric("No Data","No Data")

    # ----------------------------
    # Weekly Worker Performance
    # ----------------------------
    view_df['pH_OK'] = view_df['pH'].between(8.0,8.3)
    view_df['Salinity_OK'] = view_df['Salinity'].between(25,30)
    view_df['WorkerName'] = view_df['WorkerName'].astype(str).str.strip().str.title()
    ...
    st.subheader("Worker Performance & Water Quality Compliance (Weekly)")
    st.dataframe(
        worker_summary[['Worker_Display','pH_%','Salinity_%','ScheduledFeed_kg',
                        'ActualFeed_kg','Leftover_kg','Dead_Count','Dead_Weight_g']],
        use_container_width=True
    ...
    # ----------------------------
    # Weekly Water Quality Plot (Line) – Fixed WorkerName
    # ----------------------------
    st.subheader("Weekly Water Quality (Avg Salinity & pH)")
    weekly_df_plot = view_df.groupby(['Tank','Block'], as_index=False).agg(
        Salinity_avg=('Salinity','mean'),
        pH_avg=('pH','mean'),
        WorkerName=('WorkerName','first')  # Ensure WorkerName is present
    )
    weekly_df_plot["X_label"] = weekly_df_plot["Tank"].astype(str) + " | " + weekly_df_plot["Block"].astype(str)
    df_melt = weekly_df_plot.melt(
    ...
    # ----------------------------
    # Weekly Feed Trends (Line) – WorkerName included
    # ----------------------------
    st.subheader("Weekly Feed Trends")
    weekly_feed = view_df.groupby(['Tank','Block'], as_index=False).agg(
        ScheduledFeed_g=('ScheduledFeed_day_g','sum'),
        ActualFeed_g=('ActualFeed_day_g','sum'),
        WorkerName=('WorkerName','first')
    )
    weekly_feed["X_label"] = weekly_feed["Tank"].astype(str) + " | " + weekly_feed["Block"].astype(str)
    df_feed = weekly_feed.melt(
        id_vars=['X_label','WorkerName','Tank','Block'],
    ...
    # ----------------------------
    # Weekly Mortality Trends – WorkerName included
    # ----------------------------
    st.subheader("Weekly Mortality Trends")
    weekly_mort = view_df.groupby(['Tank','Block'], as_index=False).agg(
        Total_Dead=('DeadCount_day','sum'),
        Total_Weight=('DeadWeight_g','sum'),
        WorkerName=('WorkerName','first')
    )
    weekly_mort["X_label"] = weekly_mort["Tank"].astype(str) + " | " + weekly_mort["Block"].astype(str)
    chart = alt.Chart(weekly_mort).mark_bar().encode(
        x='X_label',
        y='Total_Dead',
...
# ==========================================
# 2️⃣ RISK TABLE (Daily / Weekly / Monthly)
# ==========================================
...
def get_alert_level(row):
    alerts = []
    if row['pH'] <= 8.0 or row['pH'] >= 8.3: alerts.append("pH")
    if row['Salinity'] <= 25 or row['Salinity'] >= 30: alerts.append("Salinity")
    if row['WaterTemperature'] <= 28 or row['WaterTemperature'] >= 30: alerts.append("Temp")
    if row['DeadCount_day'] >= 5: alerts.append("Mortality")
    
    if not alerts: return "Normal ✅"
...
def get_alert_details(row):
    details = []
    # pH
    if 8.0 <= row['pH'] <= 8.3: details.append("pH ✅")
    elif 7.9 <= row['pH'] < 8.0 or 8.3 < row['pH'] <= 8.4: details.append("pH ⚠")
    else: details.append("pH 🔴")
    # Salinity
    if 25 <= row['Salinity'] <= 30: details.append("Salinity ✅")
    elif 24 <= row['Salinity'] < 25 or 30 < row['Salinity'] <= 31: details.append("Salinity ⚠")
    else: details.append("Salinity 🔴")
    # Water Temperature
    if 28 <= row['WaterTemperature'] <= 30: details.append("Temp ✅")
    elif 27 <= row['WaterTemperature'] < 28 or 30 < row['WaterTemperature'] <= 31: details.append("Temp ⚠")
    else: details.append("Temp 🔴")
    # Mortality
    if row['DeadCount_day'] < 5: details.append("Mortality ✅")
    elif 5 <= row['DeadCount_day'] <= 6: details.append("Mortality ⚠")
    else: details.append("Mortality 🔴")
    return ", ".join(details)
...
if not view_df.empty:
    ...
    # Coloring functions
    def color_alert(val):
        if val == "Critical 🔴": return "background-color: #FF0000; color: white; font-weight: bold"
        elif val == "Warning ⚠": return "background-color: #FFA500; color: black; font-weight: bold"
        elif val == "Normal ✅": return "background-color: #90EE90; color: black; font-weight: bold"
        return ""
    def color_ph(val):
...
# Load ABW Data
try:
    abw_df = pd.read_excel("ABW.xlsx")
    abw_df.columns = abw_df.columns.str.strip()
    abw_df['Block'] = abw_df['Block'].str.strip().str.upper()
    abw_df['Tank'] = abw_df['Tank'].str.strip().str.upper()
...
c1, c2 = st.columns(2)
with c1:
    start_date = st.date_input("Start Date", value=datetime(2025, 11, 26))
with c2:
    end_date = st.date_input("End Date", value=datetime(2025, 12, 23))
...
if 'view_df' in globals() and not abw_df.empty:
    ...
    for col in ['DeadWeight_g','ActualFeed_day_g','InitialCount','LiveCount','pH','Salinity']:
    ...
    tank_df = merged_df.groupby(['Block','Tank']).agg({
        ...
        'ActualFeed_day_g':'sum','DeadWeight_g':'sum','pH':'mean','Salinity':'mean'
    }).reset_index()
    ...
    # -----------------------------
    p_list = ["ABW_start","ABW_end","InitialCount","LiveCount","ActualFeed_day_g","DeadWeight_g","Dead_Count","DeadWeight_kg","Feed_kg","Biomass_start_kg","Biomass_kg","Weight_Gain_kg","ADG (g/day)","Survival %","FCR","Avg pH","Avg Salinity"]
    ov_fcr = round(tank_df['Feed_kg'].sum()/tank_df['Weight_Gain_kg'].sum(),2) if tank_df['Weight_Gain_kg'].sum()>0 else 0
    cons_vals = [
        round(tank_df['ABW_start'].mean(),2), round(tank_df['ABW_end'].mean(),2), tank_df['InitialCount'].sum(), tank_df['LiveCount'].sum(),
        tank_df['ActualFeed_day_g'].sum(), tank_df['DeadWeight_g'].sum(), tank_df['Dead_Count'].sum(), round(tank_df['DeadWeight_g'].sum()/1000,2),
    ...
    worker_raw = tank_df.groupby('Worker').agg({
        'ABW_start':'mean','ABW_end':'mean','InitialCount':'sum','LiveCount':'sum',
    ...
    for _, row in worker_raw.iterrows():
        ...
        wvals = [round(row['ABW_start'],2),round(row['ABW_end'],2),row['InitialCount'],row['LiveCount'],
    ...
    # 6. DASHBOARD DISPLAY
    # -----------------------------
    st.subheader("Farm Consolidated Summary")
    ...
    def create_pdf(cons_df, worker_v_df, start_d, end_d):
        ...
        pdf.cell(0, 10, txt="Shrimp Farm Full Performance Report", ln=True, align="C")
